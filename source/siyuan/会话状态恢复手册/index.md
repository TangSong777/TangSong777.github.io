...
> 用途：**新会话完美续接本知识库工作**。新会话开场请说"读取[Skill速查手册](/siyuan/Skill速查手册/)与本手册继续"，AI 将据此恢复全部上下文。最后更新：2026-09-22（新增 software-list-updater Skill，并首次整理《电脑软件清单》：新增 9 项工具、跳过 11 项重复、新增子类「一/6 桌面与窗口增强」；本手册由 session-state-sync 持续同步维护）。

## 一、新会话恢复步骤

1. 读取本手册（掌握结构）+ [Skill速查手册](/siyuan/Skill速查手册/)（掌握 11 个 Skill）。
...
|software-list-updater|维护《电脑软件清单》：三列表格（序号+工具名+功能）补条目、去重、调类，刷新顶部统计总表（数量/软件名/❌️不常用数/引用链接/总数）与文末 ❌️ 备注、开机必装清单|要求添加/整理软件清单条目、为工具归类、更新 ❌️、刷新统计目录或维护开机必装清单时|
...
|会话状态恢复手册（本手册）|20260824164106-hdndq7b|
...
- **最近完成**：① 博客部署换用 GitHub Pages 方案；② 新增 software-list-updater Skill 并首次整理《电脑软件清单》（新增 9 项工具、跳过 11 项重复、新增子类「一/6 桌面与窗口增强」）；③ 全部 23 张分类表加「序号」列，文档顶部新增「分类统计总表」（23 子类行 + 合计，双括号引用可跳转；合计 82 条 / 23 个子类）；④ 统计总表增列「其中 ❌️ 不常用」并据文档实际标记刷新（❌️ 共 13 条：一 1 / 三 1 / 四 11）；⑤ 固化口径： **❌️ ＝本人不常用（有一定接触、并非不可替代的关键工具），不是已弃用**，文末新增「备注：❌️ 标记的含义」；⑥ 文末新增「开机必装清单」：21 个软件（火绒安全 → VSCode），索引性质不计入统计总表数量，每行带「所在分类」双括号引用
- **可能的中断点**：PathLib（L2）、NumPy（L2）、Windows 命令行（L2）均未脱离资料独立验证，深化任务待做；OpenCV/PyTorch/SSH/Docker 等 L0 项待学习
- **待处理（引用规范，2026-09-22 记录，用户选择暂不修复）** ：​`/学习笔记`​ 与 ​`/待学习`​ 两个目录文档共 198 处使用 Markdown 链接式引用 ​`[名称](siyuan://blocks/ID)`​，未改为双括号 ((块ID "名称"))，不产生双向链接；​`/Skill速查手册` 中 3 处为 link-checker 规则说明示例，属正常保留
