...
updated: '2026-09-01T17:31:03+08:00'
layout: page
type: siyuan-note
notebook: '学习笔记'
...
|概念|一句话|更生活的类比|
| ------| --------------------| -----------------------------|
|镜像|**打包好的模板**（程序+依赖+配置）|手机上的"安装包 .apk"|
|容器|**从模板启动的一个运行实例**|安装包装好、正在运行的 App|
|仓库|**存放镜像的网盘/商店**|应用商店（默认 Docker Hub）|

 **"拉镜像"是从哪拉？** ：默认从 **Docker Hub**（Docker 官方镜像仓库，相当于"应用商店"）拉：`docker pull python:3.11-slim` = 从商店下载"Python 3.11 精简版安装包"到本地；国内网络慢时可配镜像加速源（见第八章排障）。
...
**为什么有"免 sudo"这一行（讲透）** ：Docker 默认需要管理员权限执行，所以每条命令都要 `sudo`​。`sudo usermod -aG docker $USER`​ 把你的账号加进 `docker`​ 用户组，组内成员可免 sudo 用 docker（`-a`​=追加、`-G`​=组、`$USER`​=当前用户；**重新登录生效**）。

- **为什么默认要 sudo**：能操作 Docker ≈ 能操作 root（容器能挂载宿主机文件），所以默认"管理员才配用"；
- **加入 docker 组 = 信任该用户**：只有信得过的账号才加组；安全上等于把 root 权限的一部分交给它（Docker 守护进程漏洞可致提权），公共机器不要随便加人。
