...
编辑器默认使用单栏所见即所得模式，并以 Markdown 作为唯一保存格式；“源码”模式可以查看和修改包含 YAML 在内的完整原文。所见即所得模式只在浏览器内更新当前文档，不在输入过程中调用 Hexo；保存和上传继续使用原有的后台校验与构建流程。

“思源隐私”面板管理两类本地规则：整篇排除的文档，以及在公开副本中替换为固定黑框的已知文本。规则默认保存在 Rock 的 `/srv/blog/state/siyuan-privacy-rules.json`，不修改思源原稿，也不进入 Git；思源导入器会在生成页面、目录和引用关系前应用这些规则。
